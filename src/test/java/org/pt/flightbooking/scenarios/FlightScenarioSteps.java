...
public class FlightScenarioSteps extends ConfigurationAutomatedTest {
	...
	public void given_a_flight_search_is_submitted() {
		try {
			...
			final FlightSearchParams params = FlightSearchParams.builder().flyTo(flyTo).currency(currency).dateFrom(dateFrom).dateTo(dateTo).build();
			...
			if(response.isPresent()) {
				...
				scenarioContext.setContext(Context.QUANTITY_FLIGHTS_IN_SEARCH, response.get().getAverageFlights().entrySet().size());
	...
	public void when_the_flight_successfully_is_submitted() {
		try {
			assertThat("The status of hiker submission is not the expected ", (Boolean) scenarioContext.getContext(Context.SUBMIT_FLIGHT_SEARCH_SUCCESS), is(Boolean.TRUE));
		} catch (final Exception e) {
	...
	public void and_at_least_one_flight_exist() {
		try {
			assertThat("The status of hiker submission is not the expected ", (Integer) scenarioContext.getContext(Context.QUANTITY_FLIGHTS_IN_SEARCH) > 0 ? Boolean.TRUE : Boolean.FALSE, is(Boolean.TRUE));
		} catch (final Exception e) {
